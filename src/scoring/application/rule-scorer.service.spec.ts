...
describe('RuleScorerService', () => {
  ...
  it('flags risk rules', () => {
    const result = service.score({
      ...
      snippet: 'western union only whatsapp',
    });
  ...
  it('flags messenger-only contact', () => {
    const result = service.score({
      ...
      snippet: 'solo whatsapp per info',
    });
  ...
  it('scores listing with sparse criteria defaults', () => {
    const result = sparse.score({
      draft: { canonicalUrl: 'https://example.com/8' },
      snippet: 'random',
    });
  ...
  it('handles missing rooms area and rent', () => {
    const result = service.score({
      draft: { canonicalUrl: 'https://example.com/9' },
      snippet: 'text',
    });
    expect(result.score).toBeLessThan(50);
    expect(result.riskLevel).toBe('none');
  ...
  it('adds proximity bonus when reference configured', () => {
    const result = service.score({
      draft: {
